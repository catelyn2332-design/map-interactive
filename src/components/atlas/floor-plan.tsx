import { Check, Minus, Plus, Scan, Trash2 } from "lucide-react";
...
import { tintOf } from "@/lib/map/props";
...
export const FloorPlan = memo(function FloorPlan() {
  ...
  function onPointerUp(e: React.PointerEvent) {
    ...
    if (pointers.current.size === 0) {
      ...
      if (d?.kind === "move-mark" && d.markId && liveMark) {
        patchFixture(d.markId, { x: liveMark.x, y: liveMark.y });
        setLiveMark(null);
        drag.current = null;
        return;
      }
  ...
  const labels = useMemo(() => {
    ...
    return placed;
  }, [rooms, selectedId, query, filters, schema, livePoly]);
