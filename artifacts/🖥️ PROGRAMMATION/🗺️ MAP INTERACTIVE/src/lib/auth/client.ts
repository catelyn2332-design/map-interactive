...
/**
 * Atlas always offers real sign-in (Google / X via the Grok broker).
 * The template default `VITE_AUTH_ENABLED=false` would pin everyone to
 * `dev@example.com` and hide the account switcher.
 */
...
const LAST_ACCOUNT_KEY = "atlas-auth-last-account";
...
export function rememberAccount(account: RememberedAccount | null): void {
  if (typeof window === "undefined") return;
  try {
    if (account && account.id && account.id !== "dev-user") {
      window.localStorage.setItem(LAST_ACCOUNT_KEY, JSON.stringify(account));
...
export function readRememberedAccount(): RememberedAccount | null {
  ...
  try {
    ...
    if (!parsed || typeof parsed.id !== "string" || parsed.id === "dev-user") {
      return null;
    }
    return parsed;
  } catch {
...
export function rememberProvider(providerId: string): void {
  if (typeof window === "undefined") return;
  try {
    const prev = readRememberedAccount() ?? {
      id: "",
      displayName: null,
      primaryEmail: null,
    };
    rememberAccount({ ...prev, providerId });
  } catch {
    /* ignore */
  }
}
