// Export SVG d'Atla — copie du source src/lib/atla-svg.ts
// Génère le plateau camembert (1280×860) à partir du thème résolu.
...
export function downloadAtlaSvg(theme: ResolvedTheme) {
  const blob = new Blob([serializeAtlaSvg(theme)], { type: "image/svg+xml;charset=utf-8" });
  const url = URL.createObjectURL(blob);
  const link = document.createElement("a");
  link.href = url;
  link.download = "Atla.svg";
